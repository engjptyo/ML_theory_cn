...
<h6> - 给定一定数量的数据，模型参数控制模型过度配套，最佳服装或装备 </h6>
...
<div style = "float:right; width: 15%">
<h6> 从左上角开始, <br><br>(a) 原始数据   <br><br>(b) 装备不足[高偏差]   <br><br>(c) 最适合   <br><br>(d) 过度拟合[高差异] </h6>
...
<div style = "float:right; width: 15%">
<h6> - 对于8，相同的多项式，顶部过度拟合，底部 - 恰好与更多的数据 </h6>
<h6> - 额外的倾向于推广模型</h6>
...
- 导致该算法对您的训练数据的高度变化非常敏感  

- 导致您的模型超载数据  

- 对您的模型的训练数据带来太多噪音，对您的测试数据非常有用
...
<h6>- 对于修复数据量，培训和交叉评估的典型错误各不相同</h6>

<h6>- 如果数据是高偏差 - 更复杂的模型会降低误差如果数据高方差 - 更复杂的模型将会更多地减少错误（由于过度拟合），并且使用验证数据，性能不会很好</h6>
...
- 梯度升压机（GBM）:
      * <h4>树木数量，树木深度，学习率，采样率等<h4>
- 深度学习:
      * <h4>激活，隐层数，隐层大小，L1，L2，辍学率等<h4>
